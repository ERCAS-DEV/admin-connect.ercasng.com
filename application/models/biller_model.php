...
class Biller_model extends CI_Model {
	...
	/***  Function for Biller Name ***/
	public function biller_name($id)
	{
		//return 'FELA';
		//$this->db-select('company_name');
		$this->db->where('id', $id);
		$this->db->limit(1);
		$query = $this->db->get('biller');
		foreach ($query as $row)
		{
			$billername = $row->company_name;
		}
		return $billername;
	}
	/****** EOF *****/
	...
	public function get_ercas_service($billertbl)
	{
		$ercas_service = '';
		$split = array();
		$split = explode("_",$billertbl);
		$ercas_service = $split[1];
		
		if($ercas_service == 'collection')
		{
			$ercas_service = 'ERCASPay';
		}
		if($ercas_service == 'ecommerce')
		{
			$ercas_service = 'ERCASCentral';
		}
		if($ercas_service == 'pos')
		{
			$ercas_service = 'ERCASPOS';
		}
		return $ercas_service;
	}
	...
	//updating centralpay billers NIBSS Secret Key
	public function insert_key($data)
	{
		$this->db->where('id',$data['last_insert_id']);
		$query = $this->db->update('biller',array('secretid'=>$data['secretId'],'cp_merchant_id'=>$data['cp_merchant_id']));
